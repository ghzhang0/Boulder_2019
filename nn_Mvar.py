...
with experiment.train():
    for index_n1 in range(len(args.inputSize)):
        ...
        for index_n2 in range(len(args.hiddenSize)):
            ...
            M_exps = np.array([np.max([R_exp, i]) for i in 1 + dM_exp])
            Ms = N**(M_exps)
            for M_ind, M in enumerate(Ms):
                y = torch.tensor(np.genfromtxt('y_M_var/y_N_{}_Rexp_{}_M_{}.csv'.format(N, R_exp, M_ind), delimiter=','), device=device).float()
                ...
                for t in range(args.epochs):
                    ...
                    if k>1000:
                        ...
                        if abs(list_loss[k-1000]-list_loss[k])<0.0001:
                            np.savetxt('weights1_' + str(args.inputSize[index_n1]) + '_' + str(args.hiddenSize[index_n2]) + '_' + str(M_ind) + '.dat', w1.detach().numpy())
                            np.savetxt('weights2_' + str(args.inputSize[index_n1]) + '_' + str(args.hiddenSize[index_n2]) + '_' + str(M_ind) +'.dat', w2.detach().numpy())
                            np.savetxt('bias1_' + str(args.inputSize[index_n1]) + '_' + str(args.hiddenSize[index_n2]) + '_' + str(M_ind) +'.dat', b1.detach().numpy())
                            np.savetxt('bias2_' + str(args.inputSize[index_n1]) + '_' + str(args.hiddenSize[index_n2]) + '_' + str(M_ind) +'.dat', b2.detach().numpy())
                ...
                np.savetxt('loss_' + str(args.inputSize[index_n1]) + '_' + str(args.hiddenSize[index_n2]) + '_' + str(M_ind) +'.dat', list_loss[-500:])
