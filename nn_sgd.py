...
with experiment.train():
    for index_n1 in range(len(args.inputSize)):
        for index_n3 in range(len(args.outputSize)):
            ...
            M = N # args.outputSize[index_n3] # output dimension
            x = torch.tensor(np.genfromtxt("x_{}.csv".format(it_ind), delimiter=','), device=device).float()
            y = torch.tensor(np.genfromtxt("y_{}.csv".format(it_ind), delimiter=','), device=device).float()
